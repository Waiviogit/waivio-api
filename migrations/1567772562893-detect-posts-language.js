...
const regularExp = /(?:!?\[(.*?)\]\((.*?)\))|(<\/?[^>]+(>|$))|(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,})/g;
...
exports.up = async function up(done) {
  ...
  await cursor.eachAsync(async (doc) => {
    ...
    const res = await Post.update(
      { author: post.author, permlink: post.permlink }, { $set: { language } },
    );

    if (res.nModified) {
      successCount++;
      if (successCount % 5000 === 0) console.log(`${successCount}Posts successfully localise`);
    } else {
      failCount++;
      if (failCount % 5000 === 0)console.error(failCount);
    }
  });
  console.log(`${successCount} posts successfully updated with language!\n${failCount} posts fails on update.`);
  done();
};
...
const detectPostLanguage = (title, body, userLanguages = []) => {
  ...
  let str = `${title.replace(regularExp, '')}\n`;

  str += body.replace(regularExp, '');
  ...
  // else if matched languages not empty, get top 5 matched
  // languages, and overlap it with user languages
  const overlapLang = _
