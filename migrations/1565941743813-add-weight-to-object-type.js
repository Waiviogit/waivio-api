...
exports.up = async function up ( done ) {
    ...
    await cursor.eachAsync( async ( doc ) => {
        const [ { total_weight } ] = await WObject.aggregate( [
            { $match: { object_type: doc.name } },
            { $group: { _id: null, total_weight: { $sum: '$weight' } } }
        ] );
        const res = await ObjectType.updateOne( { _id: doc._id }, { $set: { weight: total_weight } } );

        if( res.nModified ) {
            console.log( `ObjectType ${doc.name} updated! Now "Type" has ${total_weight} weight!` );
        }
    } );
    done();
};
