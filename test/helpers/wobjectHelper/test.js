...
describe('On wobjectHelper', async () => {
  ...
  describe('On pin field', async () => {
    const pin1 = faker.random.string();
    const pin2 = faker.random.string();
    const pin3 = faker.random.string();
    const pin4 = faker.random.string();
    let obj, result;
    ...
    beforeEach(async () => {
      ...
      result = await wObjectHelper.processWobjects({
        ...
        app,
        returnArray: false,
        fields: [FIELDS_NAMES.PIN],
      });
    });

    it('should pin length eq 3', async () => {
      expect(result.pin.length).to.be.eq(3);
    });

    it('should not include 4 pin (less weight)', async () => {
      const expected = _.includes(result.pin, pin4);
