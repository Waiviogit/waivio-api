...
async function getPostsIds(type) {
...
async function updateFeedsCache() {
  // get array with ids by each language(locale)
  // array of objects {locale:'en-US', ids:['123_asdf'...]}
  const hotFeedCache = await getPostsIds('hot');
  const trendFeedCache = await getPostsIds('trending');
